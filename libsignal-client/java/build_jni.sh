...
build_desktop_for_arch () {
    ...
    echo_then_run cargo build -p libsignal-jni ${RUST_RELEASE:+--release} ${FEATURES:+--features "${FEATURES[*]}"} --target "$1"
...
export CFLAGS="-DOPENSSL_SMALL -flto=full -U_FORTIFY_SOURCE ${CFLAGS:-}"
export CXXFLAGS="-DOPENSSL_SMALL -flto=full -U_FORTIFY_SOURCE ${CXXFLAGS:-}"
...
export RUSTFLAGS="-C link-arg=-Wl,--hash-style=both --cfg curve25519_dalek_bits=\"64\" ${RUSTFLAGS:-}"
...
for abi in "${android_abis[@]}"; do
    ...
    export BINDGEN_EXTRA_CLANG_ARGS="--target=${rust_target}${ANDROID_MIN_SDK_VERSION}"
    echo_then_run cargo build -p libsignal-jni ${RUST_RELEASE:+--release} ${FEATURES:+--features "${FEATURES[*]}"} -Z unstable-options --target "$rust_target" --artifact-dir "${ANDROID_LIB_DIR}/$abi" --timings
